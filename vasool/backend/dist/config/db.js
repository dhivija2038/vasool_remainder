...
async function seedData() {
    ...
    try {
        ...
        const [existingFixedUser] = await connection.query('SELECT * FROM users WHERE username = ?', [FIXED_EMAIL]);
        if (existingFixedUser.length === 0) {
            ...
            if (legacyAdmin.length > 0) {
                ...
                await connection.query('INSERT INTO users (username, password_hash, owner_name) VALUES (?, ?, ?)', [FIXED_EMAIL, hash, 'Shop Owner']);
