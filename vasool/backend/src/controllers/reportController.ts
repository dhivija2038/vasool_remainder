...
async function calculateCustomerFine(customerId: number, connection: any): Promise<{ fine: number; daysOverdue: number }> {
  ...
  if (customers.length === 0) return { fine: 0, daysOverdue: 0 };
...
export async function getCollectionReport(req: AuthRequest, res: Response) {
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT p.*, c.customer_name, c.mobile_number 
       FROM payments p 
...
export async function getDueReport(req: AuthRequest, res: Response) {
  const { date_from, date_to } = req.query;
  if (!date_from || !date_to) {
    return res.status(400).json({ message: 'date_from and date_to parameters are required' });
  }
...
export async function getLedgerReport(req: AuthRequest, res: Response) {
  ...
  try {
    ...
    // 1. Get customer info
    const [customers] = await pool.query<any[]>('SELECT * FROM customers WHERE id = ?', [customerId]);
    ...
    const [payments] = await pool.query(
      'SELECT * FROM payments WHERE customer_id = ? ORDER BY payment_date ASC, id ASC',
      [customerId]
    );
...
export async function logReportGeneration(req: AuthRequest, res: Response) {
  ...
  try {
    const pool = getPool();
    await pool.query(
      'INSERT INTO reports (report_type, date_from, date_to) VALUES (?, ?, ?)',
      [report_type, date_from || null, date_to || null]
    );
